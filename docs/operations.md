...
| `CFSCAN_AGENT_TOKEN` | 空（禁用） | 旧共享 Agent Token，仅迁移兼容；需要时显式配置 |
| `CFSCAN_PUBLIC_WEB_URL` | `http://localhost:18081` | 终端配对 URL使用的管理台公开地址 |
| `CFSCAN_PUBLIC_AGENT_URL` | `http://localhost:18080` | Web 生成 Agent 命令使用的公开 API 地址 |
| `CFSCAN_AGENT_IMAGE` | `ghcr.io/3011/cfscan-agent:v1.1.0` | Web 展示的 Agent 容器镜像 |
| `CFSCAN_AGENT_VERSION` | `v1.1.0` | Web 展示的 Agent 推荐版本 |
...
| `CFSCAN_AGENT_TOKEN` | 空 | 旧共享鉴权模式使用；新 Agent不需要配置 |
| `CFSCAN_AGENT_NAME` | `local-agent` | 旧共享鉴权模式的名称 |
| `CFSCAN_AGENT_REGION` | `local` | 旧共享鉴权模式的地区 |
| `CFSCAN_AGENT_CONTINENT` | `local` | 旧共享鉴权模式的大洲 |
...
4. 旧 Agent迁移期间再检查共享 Token是否一致；
5. Agent 最近心跳是否在 45 秒内；
6. Agent 节点是否具备目标网络和 DNS/HTTPS 出站；
7. IPv6 扫描时节点是否具备 IPv6 路由；
8. 任务是否已停止、租约是否过期或 Agent 是否被限定。

## 安全要求

- 生产必须使用 HTTPS；
- 保持 `CFSCAN_COOKIE_SECURE=true`；
- Bootstrap 密码、旧共享 Token、一次性配对密钥和数据库 DSN 只通过 Secret 注入；
...
公开版本由 Git Tag 与 GitHub Release 共同定义。当前配对功能计划随以下版本发布：
