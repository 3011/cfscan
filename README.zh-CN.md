...
单一 HTTPS Host / Ingress
  ...
  └── /*     → React + shadcn/ui 管理台
                    ▲
              Agent 独立 Bearer Token
   亚洲 Agent   欧洲 Agent   北美 Agent
```


公网只需要一个地址。Ingress 将 `/api/*` 直接转发到 Center，其他路径转发到 Web；Agent 流量不经过 Web 容器。
...
- **冠军层**：每小时允许进入下一轮，每轮最多选择 6 个目标，其中最多 2 个是固定复测候选；
- **挑战层**：每 6 小时允许进入下一轮，每轮最多选择 3 个目标，其中最多 1 个是固定候选；
- **观察层**：每 24 小时保留一次抽样机会，每轮选择 1 个新 IP。

层级按 `Agent + 前缀 + 探测配置` 独立计算。升级使用最近 7 天的跨 IP 样本：观察层至少需要 3 个样本和 3 个不同 IP，挑战层升冠军至少需要 8 个样本和 6 个不同 IP。降级参考最近 24 小时，普通恶化需连续两个坏窗口，连续完全不可用可直接回到观察层。一次偶发抖动不会立即淘汰前缀。

`target_count` 在联赛模式中表示**每个 Agent 的单轮目标上限**。当到期前缀超过预算时，系统按冠军、挑战、观察约 `3:2:1` 交错分配目标，未被选中的前缀保持到期状态，在后续轮次继续获得机会。自动计划若没有任何到期前缀，会记录为“已跳过”而不是失败；人工“立即运行”会强制执行一轮。

管理台“最佳 IP”页面展示前缀层级和候选 IP。候选详情页按相同 Agent、IP、协议、域名、路径、端口、尝试次数和超时配置展示 24 小时、7 天或 30 天的延迟与丢包趋势。
...
CFSCAN_AGENT_IMAGE='ghcr.io/3011/cfscan-agent:v2.1.1' \
CFSCAN_AGENT_VERSION='v2.1.1' \
...
管理台、管理 API 和 Agent API 使用同一个公开地址。共享 Agent Token和旧注册接口已经删除。
...
```bash
./bin/cfscan-agent connect --server https://cfscan.example.com
```
...
自动化环境可使用 `--token-file` 或 `--token-stdin`，避免一次性密钥进入 Shell history。完整状态、Docker 示例和 HTTPS 规则见 [`docs/agent-enrollment.md`](docs/agent-enrollment.md)。
